...
pop_size = 1000000
data_pop = pd.DataFrame(index=range(pop_size), columns=range(n), dtype=np.float64)
data_pop.loc[:,:n//2] = pd.DataFrame(np.random.multivariate_normal(mean=[0]*(n//2), cov = np.eye(n//2), size=(pop_size,)))
